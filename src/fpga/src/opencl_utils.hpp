...
// 用于OpenCL配置
struct ConfigOpenCL {
	// 构造函数，初始化kernel_name="spmv_bscsr_top_k_main"、num_kernels=8、num_queues=1
	ConfigOpenCL(std::string kernel_name, unsigned int num_kernels = 1, unsigned int num_queues = 1) : kernel_name(kernel_name), num_kernels(num_kernels), num_queues(num_queues) {
		kernel = std::vector<cl::Kernel>(num_kernels);	// 初始化8个大kernel
		queue = std::vector<cl::CommandQueue>(num_queues);	// 初始化1个command queue，同时只能执行一个内核或数据传输操作。
	}
		...
		if (platform_name == "Xilinx") {
			...
			for (auto target_device_name : target_devices) {
				for (size_t j = 0; j < devices.size(); j++) {
					...
					std::cout << "We found:\t" << device_name << std::endl;
					device_name = "xilinx_u280_gen3x16_xdma_1_202211_1";
					std::cout << "Device name change to:\t" << device_name << std::endl;
...
inline int setup_opencl_and_load_kernel(ConfigOpenCL &config, bool debug = true) {
	...
	for (int i = 0; i < config.num_queues; i++) {
		// 为设备创建命令队列，使能命令的profiling功能，并设置队列的执行模式为“无序”
	...
	for (unsigned int i = 0; i < config.num_kernels; i++) {	// config.num_kernels = 8
...
inline int setup_opencl(ConfigOpenCL &config, std::vector<std::string> &target_devices, std::vector<std::string> &kernels, bool debug = true) {
	int err = find_xilinx_device(config, target_devices, kernels, debug);	// Find the target device and load the xclbin file
	err = setup_opencl_and_load_kernel(config, debug);	// Create context, command queue, program, and kernel for the selected device
