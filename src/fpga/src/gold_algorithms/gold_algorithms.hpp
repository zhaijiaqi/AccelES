...
template<typename I, typename V, typename L>
inline void spmv_coo_gold_top_k_2(coo_t<I, V> &coo, V *vec, int k, I* res_idx, L* res_val) {
	// Scatter "vec" values across a vector of size E;
	std::vector<V> vec_scattered(coo.num_nnz);
	for (int i = 0; i < coo.num_nnz; i++) {
		vec_scattered[i] = vec[coo.end[i]];
	...
	// Track the current row being processed;
	I curr_row = coo.start[0];
	L curr_row_output = (L) 0.0;
	// Track the worst result in the Top-K;
	I curr_worst_idx = 0;
	L curr_worst_val = (L) 0.0;
	...
	for (int i = 0; i < coo.num_nnz; i++) {
		...
		L pointwise_contribution = coo.val[i] * vec_scattered[i];
