...
// 加法树
// inline void adder(ap_uint<Q_AP_INT_OUT_ROW_BITWIDTH>& x, int input, int index) {    
// #pragma HLS pipeline II=1
// 	// TODO: count1 pipeline ?
...
inline void adder(ap_uint<Q_AP_INT_OUT_ROW_BITWIDTH>& x, 
									int input, 
									int index, 
									ap_uint<Q_AP_INT_OUT_ROW_BITWIDTH> tmp) {    
// #pragma HLS DATAFLOW 
	// TODO: count1 pipeline ?
	int count1 = input & ((1 << (index+1)) - 1); // count1是input的二进制表示中1的个数
	count1 = (count1 & 0x55555555) + ((count1 & 0xaaaaaaaa) >> 1);
...
// x_index译码器
inline void increment_array(int ptr, ap_uint<Q_AP_INT_OUT_ROW_BITWIDTH> x[XDECODE_SIZE]) {
	ap_uint<Q_AP_INT_OUT_ROW_BITWIDTH> tmp = x[XDECODE_SIZE-1];
#pragma HLS ARRAY_PARTITION variable=x type=complete factor=XDECODE_SIZE dim=1
    for (int i=0;i<XDECODE_SIZE;i++){
#pragma HLS UNROLL
        adder(x[i], ptr,  i, tmp);
    }
}

// 读取 数据包中的 x 坐标值（压缩行向量值）
...
#pragma HLS UNROLL
		unsigned int lower_x_range = AP_INT_ROW_BITWIDTH * j;	// 压缩行向量的起始位
		unsigned int upper_x_range = AP_INT_ROW_BITWIDTH * (j + 1) -1;	// 压缩行向量的结束位
		unsigned int block_curr = block.range(upper_x_range, lower_x_range);	// 读取压缩行向量
		buffer_out[j] = *((x_bscsr *) &block_curr);	// 读到buff中
	}
}

// 读取 UltraCSR 数据包中的 x 坐标值
inline void Q_read_block_x(q_input_block block, x_out_ultracsr buffer_out[ULTRACSR_PACKET_SIZE]) {
	int ptr = 0;
#pragma HLS INLINE
	READ_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
		unsigned int lower_x_range = Q_AP_INT_ROW_BITWIDTH * j;	// 压缩行向量的起始位
		unsigned int upper_x_range = Q_AP_INT_ROW_BITWIDTH * (j + 1) -1;	// 压缩行向量的结束位
		unsigned int block_curr = block.range(upper_x_range, lower_x_range);	// 读取压缩行向量
		// std::cout << block_curr;
		ptr |= (block_curr << j); // TODO: check correctness
	}
	// std::cout << std::endl;
	increment_array(ptr, buffer_out);
	// for (int i =0; i < ULTRACSR_PACKET_SIZE; ++i) {
...
// 读取 UltraCSR 数据包中的 y 坐标值
inline void Q_read_block_y(q_input_block block, y_ultracsr buffer_out[ULTRACSR_PACKET_SIZE]) {
#pragma HLS INLINE
	READ_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
		unsigned int lower_y_range = (Q_AP_INT_ROW_BITWIDTH * ULTRACSR_PACKET_SIZE) + Q_AP_INT_COL_BITWIDTH * j;
		unsigned int upper_y_range = (Q_AP_INT_ROW_BITWIDTH * ULTRACSR_PACKET_SIZE) + Q_AP_INT_COL_BITWIDTH * (j + 1) - 1;
		unsigned int block_curr = block.range(upper_y_range, lower_y_range);
		buffer_out[j] = *((y_ultracsr *) &block_curr);
	}
}
...
// 读取 数据包中的 val 值
inline void Q_read_block_val(q_input_block block, q_type_inout buffer_out[ULTRACSR_PACKET_SIZE]) {
#pragma HLS INLINE
	READ_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
		unsigned int lower_val_range = ((Q_AP_INT_ROW_BITWIDTH + Q_AP_INT_COL_BITWIDTH) * (ULTRACSR_PACKET_SIZE)) + Q_AP_INT_VAL_BITWIDTH * j;
		unsigned int upper_val_range = ((Q_AP_INT_ROW_BITWIDTH + Q_AP_INT_COL_BITWIDTH) * (ULTRACSR_PACKET_SIZE)) + Q_AP_INT_VAL_BITWIDTH * (j + 1) -1;
		unsigned int block_curr = block.range(upper_val_range, lower_val_range);
		// std::cout << lower_val_range << ' ' << upper_val_range << std::endl;
		// std::cout << "val: " << block_curr << std::endl;
		buffer_out[j] = *((q_type_inout *) &block_curr);
	}
}

inline void Q_read_block_pack(q_input_block block) {
#pragma HLS INLINE
	READ_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
		unsigned int lower_x_range = Q_AP_INT_ROW_BITWIDTH * j;	// 压缩行向量的起始位
		unsigned int upper_x_range = Q_AP_INT_ROW_BITWIDTH * (j + 1) -1;	// 压缩行向量的结束位
		unsigned int block_curr = block.range(upper_x_range, lower_x_range);	// 读取压缩行向量
		printf("pack: %X", block_curr);	
		unsigned int lower_y_range = (Q_AP_INT_ROW_BITWIDTH * ULTRACSR_PACKET_SIZE) + Q_AP_INT_COL_BITWIDTH * j;
		unsigned int upper_y_range = (Q_AP_INT_ROW_BITWIDTH * ULTRACSR_PACKET_SIZE) + Q_AP_INT_COL_BITWIDTH * (j + 1) - 1;
		block_curr = block.range(upper_y_range, lower_y_range);
		printf(" %X", block_curr);	
		unsigned int lower_val_range = ((Q_AP_INT_ROW_BITWIDTH + Q_AP_INT_COL_BITWIDTH) * (ULTRACSR_PACKET_SIZE)) + Q_AP_INT_VAL_BITWIDTH * j;
		unsigned int upper_val_range = ((Q_AP_INT_ROW_BITWIDTH + Q_AP_INT_COL_BITWIDTH) * (ULTRACSR_PACKET_SIZE)) + Q_AP_INT_VAL_BITWIDTH * (j + 1) -1;
		block_curr = block.range(upper_val_range, lower_val_range);
		printf(" %X\n", block_curr);	
	}
}

// 读取数据包中的 新行标记位（数据包的最后一位，论文里是数据包第一位）
...
// 读取数据包中的 新行标记位（数据包的最后一位，论文里是数据包第一位）
inline void Q_read_block_xf(q_input_block block, bool_type buffer_out[1]) {
#pragma HLS INLINE
//	unsigned int lower_xf_range = 511;
//	unsigned int upper_xf_range = 511;
//	unsigned int block_curr = block.range(upper_xf_range, lower_xf_range);
	unsigned int bit_curr = block.bit(0);
	buffer_out[0] = *((bool_type *) &bit_curr);
}
...
// Write ULTRACSR_PACKET_SIZE values to a 512 bits block, taking them from a buffer of the specified type;
inline void Q_write_block_x(q_input_block* block, int_type buffer_in[ULTRACSR_PACKET_SIZE]) {
#pragma HLS INLINE
	WRITE_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
		bool curr_val = buffer_in[j];
		unsigned int lower_x_range = Q_AP_INT_ROW_BITWIDTH * j;
		unsigned int upper_x_range = Q_AP_INT_ROW_BITWIDTH * (j + 1) -1;
		block->range(upper_x_range, lower_x_range) = *((x_ultracsr *) &curr_val);
	}
}
...
// Write ULTRACSR_PACKET_SIZE values to a 512 bits block, taking them from a buffer of the specified type;
inline void Q_write_block_y(q_input_block* block, int_type buffer_in[ULTRACSR_PACKET_SIZE]) {
#pragma HLS INLINE
	WRITE_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
		int_type curr_val = buffer_in[j];
		unsigned int lower_y_range = (Q_AP_INT_ROW_BITWIDTH * ULTRACSR_PACKET_SIZE) + Q_AP_INT_COL_BITWIDTH * j;
		unsigned int upper_y_range = (Q_AP_INT_ROW_BITWIDTH * ULTRACSR_PACKET_SIZE) + Q_AP_INT_COL_BITWIDTH * (j + 1) -1;
		block->range(upper_y_range, lower_y_range) = *((y_ultracsr *) &curr_val);
	}
}
...
// Write ULTRACSR_PACKET_SIZE values to a 512 bits block, taking them from a buffer of the specified type;
inline void Q_write_block_val(q_input_block* block, q_type buffer_in[ULTRACSR_PACKET_SIZE]) {
#pragma HLS INLINE
	WRITE_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
#if USE_FLOAT
		real_type curr_val = (real_type) buffer_in[j];
#else
		q_type curr_val = (q_type) buffer_in[j].to_int();
#endif
		unsigned int lower_val_range = ((Q_AP_INT_ROW_BITWIDTH + Q_AP_INT_COL_BITWIDTH) * (ULTRACSR_PACKET_SIZE)) + Q_AP_INT_VAL_BITWIDTH * j;
		unsigned int upper_val_range = ((Q_AP_INT_ROW_BITWIDTH + Q_AP_INT_COL_BITWIDTH) * (ULTRACSR_PACKET_SIZE)) + Q_AP_INT_VAL_BITWIDTH * (j + 1) -1;
		block->range(upper_val_range, lower_val_range) = *((val_ultracsr *) &curr_val);
	}
}

// 将在buffer_in中的新的vec加载到FPGA的block中
...
// 将在buffer_in中的新的vec加载到FPGA的block中
inline void Q_write_block_vec(vec_int_inout_ultracsr* block, q_type buffer_in[ULTRACSR_PACKET_SIZE]) {
#pragma HLS INLINE
	WRITE_BLOCK: for (int j = 0; j < ULTRACSR_PACKET_SIZE; ++j) {
#pragma HLS UNROLL
		q_type curr_val = buffer_in[j];
		unsigned int lower_val_range = QUANTIZE_B * j;
		unsigned int upper_val_range = QUANTIZE_B * (j + 1) - 1;
		block->range(upper_val_range, lower_val_range) = *((unsigned int *) &curr_val);
	}
}
