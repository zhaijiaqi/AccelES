...
if __name__ == "__main__":
    ...
    APPROXIMATE_K = 512
    NUM_TEST = 100
    rst_output_file_name = f"spmspv_precision.txt"
    ...
    with open(rst_output_file,"w") as f:
        ...
        nnz = qu.print_num_nnz(spm, "Before quantification")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            # 提交每次迭代的任务给线程池
            future_to_precision = {executor.submit(qu.run_single_spmspv_test, cols, density, spm, K, spm_qtf, B, symmetric, split, APPROXIMATE_K): i for i in range(NUM_TEST)}
            # 获取结果
