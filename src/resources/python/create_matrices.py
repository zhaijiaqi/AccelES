...
DEFAULT_OUTPUT = "../../../data/matrices_for_testing/self_generated_csr/normal_distribution_+-"
...
def create_sparse_matrix(
    ...
    with open(output_file, "w") as f:
        ...
        # val = np.random.rand(total_nnz)*2-1
        # zjq 改value服从正态分布，范围缩放到(0,1)
        val = np.random.randn(total_nnz)  
        # zjq 改value服从gamma分布，范围为(0,x)全为正数
        # val = np.random.gamma(3,2,total_nnz)
        # val = val/max(val)
