...
template <typename I, typename V, typename RV>
__global__ void light_spmv(I *cudaRowCounter, I *d_ptr, I *d_cols, V *d_val, V *d_vector, RV *d_out, I N) {
    I i;
    RV sum;
    I row;
    I rowStart, rowEnd;
...
__global__ void float_to_int8(float *in, int8_t *out, int n) {
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        out[i] = (int8_t)(in[i]);
    }
}
...
__global__ void int8_to_float(int8_t *in, float *out, int n) {
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        out[i] = (float)(in[i]);
    }
}
