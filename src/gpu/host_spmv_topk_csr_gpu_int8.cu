...
struct SpMV {
	...
	void setup(int debug) {
		...
		// cudaMalloc(&res_d, sizeof(int8_t) * num_rows);
		if (use_half_precision_gpu) {
			...
			cudaMalloc(&val_d_int8, sizeof(int8_t) * num_nnz);
			cudaMalloc(&vec_d_int8, sizeof(int8_t) * num_cols);
			// cudaMalloc(&res_d_int8, sizeof(int8_t) * num_rows);
		}	
		...
		// cudaMalloc(&res_topk_d_int8, sizeof(int8_t) * k);
		row_counter[0] = 0;
		if (use_half_precision_gpu) {
			// float_to_half<<<64, 1024, 0, stream>>>(val_d, val_d_half, num_nnz);
			// float_to_half<<<64, 1024, 0, stream>>>(vec_d, vec_d_half, num_cols);
			float_to_int8<<<64, 1024, 0, stream>>>(val_d, val_d_int8, num_nnz);
			float_to_int8<<<64, 1024, 0, stream>>>(vec_d, vec_d_int8, num_cols);
		}
		...
		if (use_half_precision_gpu) {
			...
			cusparseCreateCsr(&matrix, num_rows, num_cols, num_nnz, ptr_d, idx_d, val_d_int8, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_8I);
			cusparseCreateDnVec(&vec_cusparse, num_cols, vec_d_int8, CUDA_R_8I);
		} else {
		...
		cusparseSpMV_bufferSize(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matrix, vec_cusparse, &beta, res_cusparse, CUDA_R_32F, CUSPARSE_SPMV_ALG_DEFAULT, &buffer_size);
		CHECK_CUDA_ERROR(cudaMalloc(&cusparse_buffer, buffer_size));
	}
	...
	float operator()(int debug) {
		...
		switch(gpu_impl) {
			case CSR:
				// Use cuSPARSE;
				if (use_half_precision_gpu) {
					// std::cout << "use int8 sparse" << std::endl;
					cusparseSpMV(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matrix, vec_cusparse, &beta, res_cusparse, CUDA_R_32F, CUSPARSE_SPMV_ALG_DEFAULT, cusparse_buffer);
					// cusparseSpMV(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matrix, vec_cusparse, &beta, res_cusparse, CUDA_R_32F, CUSPARSE_SPMV_ALG_DEFAULT, cusparse_buffer);
					// light_spmv<<<num_blocks, block_size_1d, block_size_1d * sizeof(int_type), stream>>>(row_counter, ptr_d, idx_d, val_d_int8, vec_d_int8, res_d_half, num_rows);
				} else {
					cusparseSpMV(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matrix, vec_cusparse, &beta, res_cusparse, CUDA_R_32F, CUSPARSE_SPMV_ALG_DEFAULT, cusparse_buffer);
				}
				break;
		...
		if (use_half_precision_gpu && gpu_impl == CSR_LIGHTSPMV) {
			...
			// get_topk<<<1, 1024, 0, stream>>>(raw_index_ptr, res_d_int8, res_topk_d_int8, k);
			half_to_float<<<64, 1024, 0, stream>>>(res_topk_d_half, res_topk_d, k);
			// int8_to_float<<<64, 1024, 0, stream>>>(res_topk_d_int8, res_topk_d, k);
		} else {
	...
	long reset(real_type *vec_, int debug) {
		...
		if (use_half_precision_gpu) {
			// float_to_half<<<64, 1024>>>(vec_d, vec_d_half, num_cols);
			float_to_int8<<<64, 1024>>>(vec_d, vec_d_int8, num_cols);
		}
		// if (use_int8_precision_gpu) float_to_int8<<<64, 1024>>>(vec_d, vec_d_int8, num_cols);
		cudaDeviceSynchronize();
...
int main(int argc, char *argv[]) {
	...
	create_sample_vector(vec, cols, false, false, false);
	...
	for (uint i = 0; i < num_tests; i++) {
		...
		spmv_times.push_back(spmv_only_time);
	...
	if (debug) {
		...
		std::cout << "Mean read-back time=" << mean(readback_times, 2) << "±" << st_dev(readback_times, 2) << " ms" << std::endl;
		std::cout << "Mean spmv time=" << mean(spmv_times, 2) << "±" << st_dev(spmv_times, 2) << " ms" << std::endl;
		std::cout << "----------------" << std::endl;
